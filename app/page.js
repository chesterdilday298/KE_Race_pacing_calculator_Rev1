...
export default function RacePacingCalculator() {
  ...
  const getPacingZones = (raceType) => {
    console.log('🔍 getPacingZones called with raceType:', raceType);
    console.log('   Type of raceType:', typeof raceType);
    console.log('   raceType length:', raceType?.length);
    ...
    if (!result) {
      console.error('❌ NO ZONES FOUND for raceType:', raceType);
      console.log('   Available keys:', Object.keys(zones));
      // Return Olympic as fallback to prevent crash
      console.log('   ⚠️ Using Olympic Triathlon as fallback');
  ...
  const calculatePacing = () => {
    console.log('🎯 calculatePacing called!');
    console.log('   formData.raceType:', formData.raceType);
    ...
    console.log('   isTriathlon:', isTriathlon);
  ...
  const nextStep = () => {
    ...
    console.log('Pacing approach:', formData.pacingApproach);
    console.log('Race type:', formData.raceType);
    
    if (step === 1) {
      // From race category to distance selection
      console.log('Moving from step 1 to 2');
      setStep(2);
    } else if (step === 2) {
      // From distance selection to approach selection
      console.log('Moving from step 2 to 3');
      setStep(3);
    } else if (step === 3 && formData.pacingApproach === 'target') {
      console.log('Moving from step 3 to 5 (target approach)');
      setStep(5); // Skip to target time input
    } else if (step === 5 && formData.pacingApproach === 'target') {
      console.log('Step 5 target: calling calculatePacing');
      try {
        calculatePacing();
        console.log('✅ calculatePacing succeeded, moving to step 6');
        setStep(6);
      } catch (error) {
        console.error('❌ ERROR in calculatePacing:', error);
        alert('Error calculating pacing: ' + error.message);
      }
    } else if (step === 5 && formData.pacingApproach === 'fitness') {
      console.log('Step 5 fitness: calling calculatePacing');
      ...
      console.log('Default: incrementing step');
      setStep(step + 1);
    }
        ...
        {step === 4 && (
          <div className="card-enter">
            <div style={{ background: 'white', borderRadius: '16px', padding: '30px 20px', boxShadow: '0 20px 60px rgba(0,0,0,0.4)', border: `3px solid ${colors.primary}` }}>
              ...
              {/* DEBUG INDICATOR */}
              <div style={{ padding: '15px', background: '#fff3cd', border: '2px solid #ffc107', borderRadius: '8px', marginTop: '20px' }}>
                <div style={{ fontWeight: 'bold', marginBottom: '8px', color: '#856404' }}>DEBUG INFO:</div>
                <div style={{ fontSize: '13px', color: '#856404', lineHeight: '1.6' }}>
                  <div>✓ Max HR: {formData.maxHRKnown !== null ? (formData.maxHRKnown ? (formData.maxHR ? '✅ Provided' : '❌ Missing') : '✅ Will calculate') : '❌ Not selected'}</div>
                  <div>✓ Resting HR: {formData.restingHRKnown !== null ? (formData.restingHRKnown ? (formData.restingHR ? '✅ Provided' : '⚠️ Optional') : '✅ Skipped') : '❌ Not selected'}</div>
                  {raceTypes[formData.raceType]?.type === 'triathlon' && (
                    <>
                      <div>✓ CSS: {formData.cssKnown !== null ? (formData.cssKnown ? (formData.css ? '✅ Provided' : '❌ Missing') : (formData.fastest100y ? '✅ Will calculate' : '❌ Need fastest 100y')) : '❌ Not selected'}</div>
                      <div>✓ FTP: {formData.ftpKnown !== null ? (formData.ftpKnown ? (formData.ftp ? '✅ Provided' : '❌ Missing') : (formData.max20minWatts ? '✅ Will calculate' : '❌ Need max 20min watts')) : '❌ Not selected'}</div>
                    </>
                  )}
                  <div>✓ Threshold Pace: {formData.thresholdPaceKnown !== null ? (formData.thresholdPaceKnown ? (formData.thresholdPace ? '✅ Provided' : '❌ Missing') : (formData.fastest5K ? '✅ Will calculate' : '❌ Need fastest 5K')) : '❌ Not selected'}</div>
                  <div style={{ marginTop: '10px', fontWeight: 'bold' }}>
                    Button Status: {
                      ...
                      ? '❌ DISABLED (see missing items above)' 
